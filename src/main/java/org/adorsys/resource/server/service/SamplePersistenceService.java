...
public class SamplePersistenceService {
	 ...
	 *  - the store pass is a standard value derived from teh user id
	 ...
	 * 
	 */
	public void persistDocument(UserID userId, CallbackHandler userKeystoreHandler, CallbackHandler keyPassHandler, BucketName bucketName, DocumentGuardName docuGuardName,
			DocumentID documentID, DocumentContent documentContent) throws ContainerExistsException, CertificateException, ObjectNotFoundException, WrongKeystoreCredentialException, MissingKeystoreAlgorithmException, MissingKeystoreProviderException, MissingKeyAlgorithmException, IOException, UnknownContainerException, WrongKeyCredentialException, NoSuchAlgorithmException, UnsupportedEncAlgorithmException, UnsupportedKeyLengthException {
		...
		if(!containerPersistence.containerExists(bucketName.getValue())) throw new ContainerExistsException(bucketName.getValue());

		// Load key from guard.
		SecretKey secretKey = loadSecretKeyFromGuard(userId, bucketName, docuGuardName, userKeystoreHandler, keyPassHandler);
		...
		String docKeyId = parseDocKeyID(docuGuardName);
		...
		objectPersistence.storeObject(documentContent.getValue(), metaIno, docHandle, docKeyStore, docKeyId,randomHandler, encParams);
	...
	private SecretKey loadSecretKeyFromGuard(UserID userID, BucketName bucketName, DocumentGuardName docuGuardName, CallbackHandler userKeystoreHandler,
		...
		String keystoreName = userKeystoreNameFromUserId(userID); 
		
		// load user keystore from bucket.
		ObjectHandle keystoreHandle = new ObjectHandle(bucketName.getValue(), keystoreName);
	 ...
	 */
	private String parseDocKeyID(DocumentGuardName docuGuardName) {
		// TODO Auto-generated method stub
		return null;
	}
	
	private String userKeystoreNameFromUserId(UserID userID){
		// keystore name is user id + keystore
		// TODO : we have a similar idea in the class: org.adorsys.encobject.userdata.UserDataNamingPolicy
		return userID.getValue() + "_Keystore"; 
	}
